...
class ft_net(nn.Module):
    ...
    def __init__(self,savepath,vis):
    ...
    def forward(self, x):
        if self.vis: # draw features or not
...
def main():
    parser = argparse.ArgumentParser(description='Python Resnet50 features Vis')
    parser.add_argument('--imagepath', type=str, default="./data/elephant.png",
                        help='input image path')
    parser.add_argument('--savepath', type=str, default="./imgs",
                        help='save output images path')
    parser.add_argument('--vis', action='store_true',
                        help='vis features or not')
    args = parser.parse_args()
    if not os.path.exists(args.imagepath):
        print("input images not exist!")
        return
    if not os.path.exists(args.savepath):
        os.mkdir(args.savepath)

    model=ft_net(args.savepath,args.vis).cuda()
